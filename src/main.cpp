...
#if defined(NDEBUG)
# error "Squardcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xca72f0c73f6ef04de4f861d50be3172c5de44def19503413946b1109c63bfed9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Squardcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Squardcoin Signed Message:\n";
...
{
    // Squardcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Squardcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Squardcoin: 1 days
static const int64 nTargetSpacing =  5 * 60; // Squardcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Squardcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Squardcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x452c5e36ae7a0600f39f9ec61a2d9258d8fbbd5a3cf2584fd841e7a1fbea14d1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 30/Mar/2020 Coronavirus hits, Economies down, many dies";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c7ceb9b9c48b62e54c5e612a8ac75d99995d440569937e4cab5c717fb47db847bd29a13a6c72f8e2a8841df5383fd3f4c47de46b28b604438ab3b17ff1c408fd") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1585510659;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084799386;

        if (fTestNet)
        {
            block.nTime    = 1585510615;
            block.nNonce   = 387693071;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x396077def1a099e0c785d77f75bfdbcf541b816409d60d8abe0fd4d7831e955c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xfc, 0xf8, 0xd5 }; // Squardcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SquardcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SquardcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SquardcoinMiner(CWallet *pwallet)
{
    printf("SquardcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("squardcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SquardcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SquardcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SquardcoinMiner, pwallet));
}
